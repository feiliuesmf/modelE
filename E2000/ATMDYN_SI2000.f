...
C****
c
cc modify uphill air mass fluxes around steep topography
c      do 2015 j=2,jm-1
c      i = im
c      do 2010 ip1=1,im
c         xx = zatmo(ip1,j)-zatmo(i,j)
c         if(xx.eq.0.0)  go to 2007
c         DO 2005 L=1,LS1-1
cccc         if((zatmo(ip1,j)-zatmo(i,j))*pu(i,j,l).gt.0.) then
c            if(xx*pu(i,j,l).gt.0.) then
c               if(pu(i,j,l).gt.0.) then
c                  wt = (pijl(ip1,j,l)/pijl(i,j,l)-sige(l+1))/dsig(l)
c               else
c                  wt = (pijl(i,j,l)/pijl(ip1,j,l)-sige(l+1))/dsig(l)
c               endif
c               if(wt.lt.1.) then
c                  if(wt.lt.0.) wt=0.
c                  pu(i,j,l+1) = pu(i,j,l+1) + pu(i,j,l)*(1.-wt)
c                  pu(i,j,l) = pu(i,j,l)*wt
c               else
c                  go to 2007
c               endif
c            endif
...
c      do 2035 j=3,jm-1
c      do 2035 i=1,im
c         xx = zatmo(i,j)-zatmo(i,j-1)
c         if(xx.eq.0.0)  go to 2035
c         DO 2020 L=1,LS1-1
cccc         if((zatmo(i,j)-zatmo(i,j-1))*pv(i,j,l).gt.0.) then
c            if(xx*pv(i,j,l).gt.0.) then
c               if(pv(i,j,l).gt.0.) then
c                  wt = (pijl(i,j,l)/pijl(i,j-1,l)-sige(l+1))/dsig(l)
c               else
c                  wt = (pijl(i,j-1,l)/pijl(i,j,l)-sige(l+1))/dsig(l)
c               endif
c               if(wt.lt.1.) then
c                  if(wt.lt.0.) wt=0.
c                  pv(i,j,l+1) = pv(i,j,l+1) + pv(i,j,l)*(1.-wt)
c                  pv(i,j,l) = pv(i,j,l)*wt
c               else
c                  go to 2035
c               endif
c            endif
