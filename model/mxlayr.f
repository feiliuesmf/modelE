...
c
     ...
     .     totem,tosal,tndcyt,tndcys,athird,trac(kdm),trc,trcdp,
     .     flxhi,flxlo,ctrast
...
c$OMP. spe,sum1,sum2,pnew,p1,p2,buoyfl,trac,trc,trcdp)
      do 1 j=1,jj
      do 1 l=1,isp(j)
...
cdiag.ttem(k),ssal(k),dens(k)+thbase,delp(k)/onem,pres(k+1)/onem,k=1,kk)
c
...
c
      ...
      if (thknew.gt.pres(k) .and. thknew.lt.pres(k+1)) then
        ...
        q=q*q
        q=q*q
css        q=0.			! reduce mxlyr depth to nearest interface
...
c
      ...
      dens(1)=sigocn(ttem(1),ssal(1))-thbase
c
...
c$OMP PARALLEL DO PRIVATE(jb)
 ...
 38   dpmixl(i,j)=dpmixl(i,j)-(uflux(i+1,j)-uflux(i,j)
...
c --- divide layer k into 2 sublayers. upper one belongs to mixed layer
        ...
        s1=u(i,j,kn-1)
        s2=u(i,j,kn  )
        if (k.eq.kk .or. (k.lt.kk .and. dpu(i,j,kn+1).lt.onemm)) then
          s3=2.*s2-s1
        else
          s3=u(i,j,kn+1)
        end if
...
c --- divide layer k into 2 sublayers. upper one belongs to mixed layer
        ...
        s1=v(i,j,kn-1)
        s2=v(i,j,kn  )
        if (k.eq.kk .or. (k.lt.kk .and. dpv(i,j,kn+1).lt.onemm)) then
          s3=2.*s2-s1
        else
          s3=v(i,j,kn+1)
        end if
