...
c      HCHO, Alkenes, and CO per rxn, correct here following Houweling
      do L=1,maxl
       prod(n_CO,L)=prod(n_CO,L)-0.63*chemrate(35,L)
       prod(n_HCHO,L)=prod(n_HCHO,L)-0.36*chemrate(35,L)
       prod(n_HCHO,L)=prod(n_HCHO,L)-0.39*chemrate(30,L)
       prod(n_Alkenes,L)=prod(n_Alkenes,L)-0.42*chemrate(30,L)
       prod(n_HCHO,L)=prod(n_HCHO,L)-0.10*chemrate(31,L)
...
c     If NOx in equil with N2O5, HO2NO2 or PAN, remove from changes
     ...
     &chemrate(iN2O5form,L).gt.y(n_NOx,L))then
        dest(n_NOx,L)=dest(n_NOx,L)+2.*chemrate(iN2O5form,L)
        prod(n_NOx,L)=prod(n_NOx,L)-2.*(chemrate(iN2O5decomp,L)
     &   +photrate(7,L))
       endif
...
c     If BrOx in equil with HOBr or BrONO2, remove from changes
     ...
     &chemrate(104,L).gt.0.5*y(n_BrOx,L))then
...
c     If ClOx in equil with HOCl or ClONO2, remove from changes
     ...
     &chemrate(103,L).gt.0.8*y(n_ClOx,L))then
...
c      add in production of NO and NO2 from NO3
       rNO3prod=rNO3prod*ss(6,L,I,J)/(ss(5,L,I,J)+ss(6,L,I,J)+1.)
...
c
     ...
     *        2.*(y(n_N2O5,lprn)-(rr(iN2O5form,lprn)*y(nNO3,lprn)*
     *        y(nNO2,lprn))/
     ...
     *    ,chemrate(73,lprn)
     ...
     &chemrate(104,lprn).gt.0.5*y(n_BrOx,lprn))then
     ...
     *    ,chemrate(63,lprn)
     ...
     &chemrate(103,lprn).gt.0.8*y(n_ClOx,lprn))then
...
c     *   -ss(25,Lz,i,j)*y(nBrO,Lz)
        ...
        write(*,'(i3,1x,5(f7.2,1x),6(e9.2,1x))') Lz,100.*sumC/sumA,
     *   100.*sumN/sumA,100.*sumH/sumA,100.*sumB/sumA,100.*sumO/sumA,
     *   rr(26,Lz)*y(nNO2,Lz)*y(nO,Lz),
...
c        N2O5 is thermally unstable, has a very short lifetime)
     ...
     &     (rr(iN2O5decomp,L)*y(nM,L)+ss(7,L,I,J)+1.d-12)
           ...
           change(I,J,L,igas)=(rnewval-y(n_N2O5,L))
           if(change(I,J,L,igas).gt.0.33*y(nNO2,L))change(I,J,L,igas)=
     &      0.33*y(nNO2,L)
           change(I,J,L,igas)=change(I,J,L,igas)*conc2mass
         endif
c
c        Conserve NOx with respect to N2O5
     ...
     &    (rr(iN2O5decomp,L)*y(nM,L)+ss(7,L,I,J)+1.d-12)
          if(rnewval.lt.1.)rnewval=1.
           ...
           if(changeX.gt.0.33*y(nNO2,L))changeX=0.33*y(nNO2,L)
          change(I,J,L,igas)=
     &    change(I,J,L,igas)-changeX*conc2mass
         endif
c
c        Set HO2NO2 to equil when necessary
     ...
     *    +ss(10,L,I,J)+ss(11,L,I,J)+1.d-12)
          if(rnewval.lt.1.)rnewval=1.
           change(I,J,L,igas)=(rnewval-y(n_HO2NO2,L))
           if(change(I,J,L,igas).gt.0.33*y(nNO2,L))change(I,J,L,igas)=
     *      0.33*y(nNO2,L)
           change(I,J,L,igas)=change(I,J,L,igas)*conc2mass
         endif
...
c        Conserve NOx with respect to HO2NO2
     ...
     *    +ss(10,L,I,J)+ss(11,L,I,J)+1.d-12)
          ...
          if(changeX.gt.0.33*y(nNO2,L))changeX=0.33*y(nNO2,L)
          change(I,J,L,igas)=change(I,J,L,igas)-
     *     changeX*conc2mass
         endif
c
...
c         PAN is thermally unstable, has a very short lifetime)
     ...
     *    (rr(iPANdecomp,L)*y(nM,L)+ss(15,L,I,J)+1.d-12)
          if(rnewval.lt.1.)rnewval=1.
          change(I,J,L,igas)=(rnewval-y(n_PAN,L))
          if(change(I,J,L,igas).gt.0.33*y(nNO2,L))change(I,J,L,igas)=
     &     0.33*y(nNO2,L)
          change(I,J,L,igas)=change(I,J,L,igas)*conc2mass
         endif
...
c        Conserve NOx with respect to PAN
     ...
     *    (rr(iPANdecomp,L)*y(nM,L)+ss(15,L,I,J)+1.d-12)
          ...
          if(changeX.gt.0.33*y(nNO2,L))changeX=0.33*y(nNO2,L)
          change(I,J,L,igas)=change(I,J,L,igas)-changeX*
     *     conc2mass
         endif
c
#ifdef Shindell_Strat_chem
c        Set HOBr to equilibrium when necessary
     ...
     *     (ss(24,L,i,j)+1.d-12)
          ...
          if(change(I,J,L,igas).gt.0.5*y(nBrO,L))change(I,J,L,igas)=
     *     0.5*y(nBrO,L)
          change(I,J,L,igas)=change(I,J,L,igas)*conc2mass
         endif
c
c        Conserve BrOx with respect to HOBr
     ...
     *     (ss(24,L,i,j)+1.d-12)
          ...
          if(changeX.gt.0.5*y(nBrO,L))changeX=0.5*y(nBrO,L)
          change(I,J,L,igas)=change(I,J,L,igas)-
     *     changeX*conc2mass
         endif
c
c        Set BrONO2 to equilibrium when necessary
     ...
     *     (ss(23,L,i,j)+1.d-12)
          if(rnewval.lt.1.)rnewval=1.
          change(I,J,L,igas)=(rnewval-y(n_BrONO2,L))
          if(change(I,J,L,igas).gt.0.5*y(nBrO,L))change(I,J,L,igas)=
     *     0.5*y(nBrO,L)
          change(I,J,L,igas)=change(I,J,L,igas)*conc2mass
         endif
...
c        Set ClONO2 to equilibrium when necessary
     ...
     *     (ss(22,L,i,j)+rr(65,L)*y(nO,L)+1.d-12)
          if(rnewval.lt.1.)rnewval=1.
          change(I,J,L,igas)=(rnewval-y(n_ClONO2,L))
          if(change(I,J,L,igas).gt.0.3*y(nClO,L))change(I,J,L,igas)=
     *     0.3*y(nClO,L)
          if(-change(I,J,L,igas).gt.0.8*y(n_ClONO2,L))
     *     change(I,J,L,igas)=-0.8*y(n_ClONO2,L)
          change(I,J,L,igas)=change(I,J,L,igas)*conc2mass
         endif
c
c        Conserve ClOx with respect to ClONO2
     ...
     *     (ss(22,L,i,j)+rr(65,L)*y(nO,L)+1.d-12)
          ...
          if(changeX.gt.0.3*y(nClO,L))changeX=0.3*y(nClO,L)
          if(-changeX.gt.0.8*y(n_ClONO2,L))changeX=-0.8*y(n_ClONO2,L)
          change(I,J,L,igas)=change(I,J,L,igas)-changeCl*
     *     conc2mass
         endif
c
...
c        Set HOCl to equilibrium when necessary
     ...
     *     (ss(21,L,i,j)+rr(55,L)*y(nO,L)+1.d-12)
          if(rnewval.lt.1.)rnewval=1.
          change(I,J,L,igas)=(rnewval-y(n_HOCl,L))
          if(change(I,J,L,igas).gt.0.3*y(nClO,L))change(I,J,L,igas)=
     *     0.3*y(nClO,L)
          change(I,J,L,igas)=change(I,J,L,igas)*conc2mass
         endif
...
c       First check for nitrogen loss > 100%
     ...
     &  change(I,J,L,n_BrONO2)=1.-trm(I,J,L,n_BrONO2)
#endif
c
...
c
     ...
     *      y(nH2O,lprn),'(',1.E6*y(nH2O,lprn)/y(nM,lprn),' ppmv)'
         else
