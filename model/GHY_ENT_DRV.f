...
!     &    veg_srht=>srht,veg_pres=>pres,veg_ch=>ch,veg_vsm=>vsm !ia
  ...
  !   &     , evap_max,fr_sat,uocean,vocean,psurf,trhr0
     ...
     *     ,HR_IN_MONTH,NDIUVAR,adiurn,NDIUPT,adiurn_dust
#ifndef NO_HDIURN
     &     ,hdiurn
#endif
#ifdef TRACERS_ON
...
cddd            if (cond_scheme.eq.2) then
cddd              aalbveg0 = 0.d0
cddd              sfv=0.d0
cddd              do iv=1,11
cddd                if ( iv==9 .or. iv==10 ) cycle
cddd                fvp=vdata(i,j,iv+1)
cddd                sfv=sfv+fvp
cddd                aalbveg0 = aalbveg0 + fvp*(ALBVNH(iv+1,1,northsouth))
cddd                !write (99,*) 'fvp',fvp
cddd                !write (99,*) 'ALBVNH',ALBVNH(iv+1,1,northsouth)
cddd              end do
cddd              aalbveg(i,j) = 0.08D0
cddd              if(sfv.gt.0.) aalbveg(i,j) = aalbveg0/sfv !nyk
cddd             !write (99,*) 'daily aalbveg', aalbveg(i,j)
cddd            end if
          !if (fearth(i,j).gt.0.) then
            !-----------------------------------------------------------
...
!!! if average albedo is still needed it should be computed elsewhere
     ...
     &        ,J_STRT_HALO=J_0H,J_STOP_HALO=J_1H)
      ...
      do j=J_0,J_1
        do i=I_0,I_1
          ...
          call ent_get_exports( entcells(i,j),
     &         fraction_of_vegetated_soil=fbv(2) )
          fbv(1) = 1. - fbv(2)
          !fbv(1) = afb(i,j)
          !fbv(2) =1.d0 - fbv(1)
