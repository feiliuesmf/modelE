...
c       flngth(k)=diapyc*sqrt(ennsq) * baclin*froglp * onem
c -----------------------------------------------------------------------
...
c
cdiag     if (i.eq.itest.and.j.eq.jtest .and. clip(k  ).lt.1.)
cdiag.     write (lp,'(i3,a,5es10.2)') k,'  pdot,dp,flxu,flxl,clip=',
cdiag.      pdot(k)/onem,dp(i,j,kn-1)/onem,flxu(k)/onem,
cdiag.       flxl(k-1)/onem,clip(k  )
c
...
c
cdiag     if (i.eq.itest.and.j.eq.jtest .and. clip(k-1).lt.1.)
cdiag.     write (lp,'(i3,a,5es10.2)') k,'  pdot,dp,flxu,flxl,clip=',
cdiag.      pdot(k)/onem,dp(i,j,kn  )/onem,flxu(k)/onem,
cdiag.       flxl(k-1)/onem,clip(k-1)
c
        end if
      end if
 42   continue
c
      ...
      kn=k+nn
c
cdiag if (i.eq.itest.and.j.eq.jtest .and. clip(k  ).lt.1.) then
cdiag  write (lp,'(i8,2i4,i3,a,4f8.4,es10.2)') nstep,i,j,k-1,
cdiag.  ' flxu,flxl,clip:',flxu(k-1)/onem,flxl(k-1)/onem,
cdiag.   flxu(k-1)*clip(k-1)/onem,flxl(k-1)*clip(k-1)/onem,clip(k-1)
cdiag  write (lp,'(i19,16x,4f8.4,es10.2)') k  ,
cdiag.  flxu(k  )/onem,flxl(k  )/onem,
cdiag.   flxu(k  )*clip(k  )/onem,flxl(k  )*clip(k  )/onem,clip(k  )
cdiag  write (lp,'(i19,16x,4f8.4,es10.2)') k+1,
cdiag.  flxu(k+1)/onem,flxl(k+1)/onem,
cdiag.   flxu(k+1)*clip(k+1)/onem,flxl(k+1)*clip(k+1)/onem,clip(k+1)
cdiag end if
c
...
c
      ...
      if (dp(i,j,kn).lt.-onemm) then
        print '(a,3i5,f9.4)','dp<0 at',i,j,k,dp(i,j,kn)/onem
        stop
      end if
c
