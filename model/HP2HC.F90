...
module hp2hc
	...
	implicit none
	private
	public :: hp_to_hc_precip_li, hp_to_hc
...
implicit none
	...
	nmat = size(ival_to_oval%vals)	! Number of non-zero elements in the matrix
	do i=1,nmat
		...
		oval%(:,ck)%p(ci,cj) = oval%(:,ck)%p(ci,cj) + &
			val * ival(:,rk)%p(ri,rj)
	end do
end subroutine multiply_by_bundle

! ----------------------------------------------------
subroutine bundle_arrays_init_li(atmglas, vars)
	...
	if iv.neq.nvars then
		write (6,*) 'bundle_arrays_precip_li: nvars must be set to',iv
		call stop_model( 'bundle_arrays_precip_li::nvars too big or small', 255 )
	end if
end subroutine bundle_arrays_init_li
! ---------------------------------------------------------
subroutine bundle_arrays_precip_li(atmglas, vars)
	USE EXCHANGE_TYPES, only : atmgla_xchng_vars
	type(atmgla_xchng_vars), dimension(:) :: atmglas
	type(double_2_ptr), dimension(:,:), allocatable :: vars

	integer :: nhc, iv, ih
	integer :: nvars = 7	! Must match below
#ifdef TRACERS_ON
	ntm = size(atmglas%gtracer,1)	! Or any variable with ntm dimension
...
subroutine bundle_arrays_precip_li
! ---------------------------------------------------------
subroutine bundle_arrays_ground_li(atmglas, vars)
	USE EXCHANGE_TYPES, only : atmgla_xchng_vars
	type(atmgla_xchng_vars), dimension(:) :: atmglas
	type(double_2_ptr), dimension(:,:), allocatable :: vars
...
end subroutine bundle_arrays_ground_li
! ---------------------------------------------------------
subroutine bundle_arrays_surface_landice(atmglas, vars)
	USE EXCHANGE_TYPES, only : atmgla_xchng_vars
	type(atmgla_xchng_vars), dimension(:) :: atmglas
	type(double_2_ptr), dimension(:,:), allocatable :: vars
...
end subroutine bundle_arrays_surface_landice
! ---------------------------------------------------------
...
#endif	// GLINT2 (for the entire module)
