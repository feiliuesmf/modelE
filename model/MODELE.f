...
C**** Also find LCSDRAG, highest level up to which C_Sdrag is applied;
C**** in this case pick the lower level in case of an edge value !
      LSDRAG=LM ; LPSDRAG=LM ; LCSDRAG=LM
      DO L=1,LM
     ...
     *      PTOP+PSFMPT*SIGE(L)+1d-5.gt.PP_SDRAG) LPSDRAG=L
        IF (PTOP+PSFMPT*SIGE(L+1)-1d-5.lt.P_CSDRAG .and. PTOP+
     *      PSFMPT*SIGE(L)+1d-5.gt.P_CSDRAG.and.LCSDRAG.eq.LM) LCSDRAG=L
      END DO
      WRITE(6,*) "Levels for L_SDRAG =",LSDRAG ,"->",LM
      WRITE(6,*) "Levels for L_SDRAG =",LPSDRAG,"->",LM," near poles"
      WRITE(6,*) "Levels for C_SDRAG =",LS1,"->",LCSDRAG
      RETURN
C****
...
C****   Add obs. H2O generated by CH4(*H2ObyCH4) using a 2 year lag
        ...
        do j=1,jm
        do i=1,im
          q(i,j,l)=q(i,j,l)+xCH4*dH2O(j,l)*byAM(l,i,j)
#ifdef TRACERS_WATER
C**** Add water to relevant tracers as well
     ...
     +                tr_H2ObyCH4(n)*xCH4*dH2O(j,l)*dxyp(j)
              end select
            end if
          end do
#endif
