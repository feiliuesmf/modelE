...
C****
     ...
     *     sqrtpx=>sqrtp
      ...
      COMMON/WORK2/W(IM,JM,LM),PHIE(IM,JM,LM-1),
     ...
     *  FC(2,IMH+1)
c      COMMON/MCDIA/AQ1(IM,JM,LM),AQ2(IM,JM,LM),CLDDEP(IM,JM)
c     *            ,CLDSLW(IM,JM),UQP(IM,JM,LM),VQP(IM,JM,LM)
      CHARACTER*16 TITLE
...
C****
     ...
     *  THSEC(IM),PSEC(IM),SHETH(LM)
      COMMON/WORK5/DUT(IM,JM,LM),DVT(IM,JM,LM),
     *  X1(IM),ZX(IM,JM,LM),XXXA(2*JM*LM),STB(IM,JM,LM),XXXB(2*JM*LM),
...
C**** TRANSFER RATES FOR KINETIC ENERGY IN THE DYNAMICS
  ...
  110 KE(N,1)=0.
      ...
      IF(K.EQ.IZERO) CALL FFT(DUT(1,J,L),FA,FB)
      IF(K.EQ.LM)    CALL FFT(DVT(1,J,L),FA,FB)
      DO 120 N=1,NM
