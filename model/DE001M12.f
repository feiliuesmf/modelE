...
C****
     ...
     &     DXYV,DYP,FCOR,IMAXJ,RAVPN,RAVPS,SINP
      ...
      USE DAGCOM, only : aj,bj,cj,rj,jreg,apj,ajl,asjl,ail,
     &     aij,
...
C**** NUMBERS ACCUMULATED FOR A SINGLE LEVEL
      ...
      RJ(JR,22)=RJ(JR,22)+(TX(I,J,1)-273.16)*DXYPJ
      SCOVL=0.
      ...
      CSCOV=CSCOV+SCOVOI
      RJ(JR,31)=RJ(JR,31)+(SCOVL+SCOVOI)*DXYPJ
      PI(J)=PI(J)+P(I,J)
...
C     RHPI=0.
      ...
      RJ(JR,63)=RJ(JR,63)+Q(I,J,L)*PIJ*DSIG(L)*DXYPJ
      RJ(JR,21)=RJ(JR,21)+(TX(I,J,L)-273.16)*DSIG(L)*BYSDSG*DXYPJ
...
C****
     ...
     &     TOFDY0,XLABEL
      ...
      USE DAGCOM, only : aj,bj,cj,rj,jreg,kdiag,namreg,nreg,kaj
      IMPLICIT NONE
...
C**** CALCULATE THE DERIVED QUANTITIES
  100 BYA1=1./(IDACC(1)+1.D-20)
      ...
      RJ(JR,4)=RJ(JR,2)-RJ(JR,6)
      RJ(JR,7)=RJ(JR,70)+A2BYA1*RJ(JR,9)/DTSRCE
      RJ(JR,8)=RJ(JR,71)+A2BYA1*RJ(JR,9)/DTSRCE
      RJ(JR,10)=RJ(JR,2)+RJ(JR,7)
      RJ(JR,11)=RJ(JR,3)+RJ(JR,8)
      RJ(JR,12)=A1BYA2*RJ(JR,6)*DTSRCE+RJ(JR,9)
      RJ(JR,16)=RJ(JR,41)+RJ(JR,42)
      RJ(JR,20)=RJ(JR,61)+RJ(JR,62)
      RJ(JR,44)=RJ(JR,12)+RJ(JR,13)+RJ(JR,14)+RJ(JR,39)-RJ(JR,40)
      RJ(JR,56)=RJ(JR,15)+RJ(JR,43)
  200 RJ(JR,60)=IDACC(2)*SAREA(JR)*RJ(JR,80)/(RJ(JR,58)+1.D-20)
      DO 210 J=1,JM
...
C****
     ...
     *  IDAY,IHOUR,JDATE,JMONTH,JYEAR,TAU,TAUDIF
      ...
      DO 520 JR=1,23
      FLAT(JR)=RJ(JR,N)*SCALE(N)*BYIACC/SAREA(JR)
...
C**** CALCULATE AND PRINT ALBEDOS FOR REGIONAL STATISTICS
  600 DO 620 KA=1,10
      ...
      IF (KA.LE.1.OR.KA.EQ.4) AMULT=-1.
      DO 610 JR=1,23
      FLAT(JR)=AMULT*(100.*RJ(JR,NN)/(RJ(JR,ND)+1.D-20)-50.)+50.
...
C---- Constants and arrays for post processing
      DOUBLE PRECISION :: XIL(IM,LM),ZONAL(LM)
