...
c    &             ,BTEMPW
      ...
      INTEGER I,J,L,K,KR,LR,IMAX,IM1,JR,IH,INCH,JK,IT,iy
...
C****   Add obs. H2O generated by CH4 using a 2 year lag
        ...
        if (QCHECK) write(6,*) 'using ch4 from year',iy-1+ghgyr1
        do l=1,lm
        do j=1,jm
        do i=1,im
          q(i,j,l)=q(i,j,l)+DTsrc*ghgam(3,iy)*dH2O(j,l)*byAM(l,i,j)
        end do
        end do
        end do
      end if
...
C**** vertical integration to model layers
        ...
        do l=1,lm
          ...
          if(dh.lt.0.) dh=0.
          dh2o(j,l) = 1.d-6*dh/1.7d0/365./86400. !->(kg/m^2/ppm_CH4/sec)
        end do
      end do
      return
