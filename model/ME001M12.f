...
CBUDG*  QTABLE(27,9,3),EHIST(20),KEYDS(42)
     ...
     *  ,SVWMX(IM,JM,LM),SVLAT(IM,JM,LM),CLDSAV(IM,JM,LM)
      PARAMETER (NGM=6, XSI1=0.5, XSI2=0.5, XSI3=0.5, XSI4=0.5)
      COMMON/SOILS3/GHDATA(IM,JM,4*NGM+5)
      COMMON/RDATA/ROUGHL(IM,JM),FSF(IM,JM,4)
      CHARACTER C*4,CYEAR*4,CMMND*80
      DIMENSION C(39),JC(100),RC(161)
      EQUIVALENCE (JC,IM0),(C,XLABEL,LABEL1),(RC,TAU)
      CHARACTER*8 :: LABSSW,OFFSSW = 'XXXXXXXX' ,LABEL1*16
...
C**** WRITE RESTART INFORMATION ONTO DISK
  120 CALL RFINAL (IRAND)
      REWIND KDISK
      WRITE (KDISK) TAU,JC,C,RC,KEYNR,U,V,T,P,Q,ODATA,GDATA,
     *  GHDATA,BLDATA,
...
C**** KCOPY > 1 : SAVE THE RESTART INFORMATION
     ...
     *        FORM='UNFORMATTED')
         WRITE (30) TAU,JC,C,RC,KEYNR,U,V,T,P,Q,ODATA,GDATA,
     *  GHDATA,BLDATA,
     *  uabl,vabl,tabl,qabl,eabl,cm,ch,cq,ipbl,
     *  TTOLD,QTOLD,SVLHX,RHSAV,WM,CLDSAV,
     *  TX,TY,TZ,TXX,TYY,TZZ,TXY,TZX,TYZ,
...
C**** SAVE THE DIAGNOSTIC ACCUM ARRAYS IN SINGLE PRECISION
  685    CONTINUE
     ...
     *         FORM='UNFORMATTED')
         WRITE (30) SNGL(TAU),JC,C,(SNGL(RC(I)),I=1,161),KEYNR,
...
C****
      ...
      INCLUDE 'E001M12.COM'
      COMMON U,V,T,P,Q
      DIMENSION JC(100),C(39),RC(161),  JC1(100),C1(39),RC1(161)
      EQUIVALENCE (JC,IM0),(C,XLABEL),(RC,TAU)
      CHARACTER*4 C,C1,RUNID
      parameter (npbl=8)
      common /socabl/uabl(npbl,im,jm,4),vabl(npbl,im,jm,4),
     2               tabl(npbl,im,jm,4),qabl(npbl,im,jm,4),
     3               eabl(npbl,im,jm,4),
     ...
     *  ,SVWMX(IM,JM,LM),SVLAT(IM,JM,LM),CLDSAV(IM,JM,LM)
     ...
     & ,SNOWD(2),GWS(0:NGM,2),GTP(0:NGM,2),FICE(0:NGM,2),GHOUR,COSDAY
     & ,SINDAY,GDZ(NGM)
      LOGICAL :: redoGH = .FALSE.,iniPBL = .FALSE.
      COMMON/SOILS2/SDATA(IM,JM,11*NGM+1)
      COMMON/SOILS3/GHDATA(IM,JM,4*NGM+5)
      COMMON/RDATA/ROUGHL(IM,JM),FSF(IM,JM,4)
...
C****     ISTART=3: C ARRAY IS BUILT UP FROM DEAULTS AND NAMELIST
  300 READ (9,ERR=800,END=810) TAUX,JC1,C1,RC1,KEYNR,U,V,T,P,Q,ODATA,
     2  GDATA,GHDATA,BLDATA,
     *   uabl,vabl,tabl,qabl,eabl,cm,ch,cq,ipbl,
...
C***********************************************************************
  320 READ(9,ERR=800)TAUX,JC,C1,RC,KEYNR,U,V,T,P,Q,ODATA,
     *  GDATA,GHDATA,BLDATA,
     *  uabl,vabl,tabl,qabl,eabl,cm,ch,cq,ipbl,
...
C**** ISTART=5; initial start from run B140 (snow ages not yet in GDATA)
  322 READ (9,ERR=800,END=810) TAUX,JC1,C1,RC1,KEYNR,U,V,T,P,Q,ODATA,
     2  GDATA,GHDATA,BLDATA,
     3  TTOLD,QTOLD,SVLHX,RHSAV,WM,CLDSAV,
     4  TX,TY,TZ,TXX,TYY,TZZ,TXY,TZX,TYZ,
     5  QX,QY,QZ,QXX,QYY,QZZ,QXY,QZX,QYZ,
...
C**** ISTART=6 ; start from run B120 (no QUS, only 1 snow age)
  325 READ(9,ERR=800)TAUX,JC1,C1,RC1,KEYNR,U,V,T,P,Q,ODATA,
     *  GDATA,GHDATA,BLDATA,
     2  TTOLD,QTOLD,SVLHX,RHSAV,WM,CLDSAV,
...
C**** RESTART ON UNIT KDISK
  450 KDISK0=KDISK
  460 READ (KDISK0,ERR=840) TAUX,JC,C,RC,KEYNR,U,V,T,P,Q,ODATA,GDATA,
     2  GHDATA,BLDATA,
...
C****
     ...
     5               ipbl(im,jm,4)
      COMMON/SOILS/   PR,HTPR,PRS,HTPRS,GW(0:NGM,2),HT(0:NGM,2)
     & ,SNOWD(2),GWS(0:NGM,2),GTP(0:NGM,2),FICE(0:NGM,2),GHOUR
     & ,COSDAY,SINDAY,GDZ(9*NGM+1)
