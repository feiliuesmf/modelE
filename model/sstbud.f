...
c
        ...
        protxt(nproc)=text
c
cdiag   write (lp,'(i9,2i4,a,f9.3)') nstep,itest,jtest,protxt(nproc),
...
c$OMP END PARALLEL DO
c
cdiag   write (lp,'(33x,f9.3)') sstold(itest,jtest)
      end if
c
      if (mod(time+.001,diagfq).gt..002 .or. nproc.lt.maxpro)
     .  return
c
...
c
 ...
 3    continue
      ...
      stop
 4    write (string,'(a7,i6.6,''-'',i6.6)')'sstbud.'
     .   ,int(time+.001-diagfq),int(time+.001)
...
c
     ...
     .   protxt(n)
      call zebra(tndcy(1,1,n),idm,ii1,jj)
c
c$OMP PARALLEL DO
      do 6 j=1,jdm
      do 6 i=1,idm
