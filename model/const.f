...
C**** Numerical constants
      ...
      real*8,parameter :: pi = 3.1415926535897932d0 !@var pi    pi
      real*8,parameter :: twopi = 2d0*pi            !@var twopi 2*pi
      real*8,parameter :: radian = pi/180d0         !@var radian pi/180
!@var rt2,byrt2   sqrt(2), 1/sqrt(2)
      real*8,parameter :: rt2 = 1.4142135623730950d0
      real*8,parameter :: byrt2 = 0.70710678118654752d0
!@var rt3,byrt3   sqrt(3), 1/sqrt(3)
      real*8,parameter :: rt3 = 1.7320508075688772d0
      real*8,parameter :: byrt3 = 0.57735026918962576d0
!@var rt12,byrt12   sqrt(12), 1/sqrt(12)
      real*8,parameter :: rt12 = 3.4641016151377546d0
      real*8,parameter :: byrt12 = 0.28867513459481288d0

      real*8,parameter :: by3 = 0.33333333333333333d0   !@var by3   1/3
      real*8,parameter :: by9 = 0.11111111111111111d0   !@var by9   1/9
      real*8,parameter :: by12 = 0.83333333333333333d-1 !@var by12  1/12

C**** Physical constants

!@var sday  sec per day (s)
      real*8,parameter :: sday = 86400.

!@var stbo Stefan-Boltzmann constant (W/m^2 K^4) 
c      real*8,parameter :: stbo =5.67051d-8 !current best estimate
      real*8,parameter :: stbo = .5672573d-7

!@var lhe   latent heat of evap (J/kg)
c**** There should be a temperature dependence (Gill):
c**** lhe = 2.5008d6 - 2.3d3 T (in C) 
      real*8,parameter :: lhe = 2.5d6  
!@var lhm   latent heat of melt (J/kg)
c**** There should be a temperature dependence (Gill):
c**** lhe = 334590 + 2.05d3 T (in C) 
      real*8,parameter :: lhm = 3.34d5
!@var lhs  latent heat of sublimation (J/kg)
      real*8,parameter :: lhs = lhe+lhm

!@var rhow density of pure water (kg/m^3)
      real*8,parameter :: rhow = 1d3
!@var rhoi density of pure ice (kg/m^3)
      real*8,parameter :: rhoi = 916.6d0

!@var tf freezing point of water at 1 atm (K)
      real*8,parameter :: tf = 273.16   !d0
!@var bytf 1/tf (K^-1)
      real*8,parameter :: bytf = 1d0/tf

!@var shw heat capacity of water (at 20 C) (J/kg C)
      real*8,parameter :: shw  = 4185.
!@var byshw 1/shw 
      real*8,parameter :: byshw = 1d0/shw

!@var rgas gas constant (J/K kg)
c**** for values of CO2 much larger than present day (4x conc)
c**** the molecular weight of dry air M_A could change
c**** RGAS = R/M_A = 1000* 8.314510 J/mol K /28.9655 g/mol (US Stand. Atm.)
c**** assuming M_w for O2 = 31.9988 and CO2 = 44.00995 and
c**** current percentage 20.946% and 0.0350% 
...
!@var omega earth's rotation rate (s^-1)
