...
c
     ...
     .    ,hyc_pechg1,hyc_pechg2,q,sum1,sum2,sum3
...
c
      ...
      real*4 convec_time,convec_total_time,convec_avg_time
      real*4 diapfl_time,diapfl_total_time,diapfl_avg_time
      ...
      data convec_total_time,diapfl_total_time /0.0,0.0/
      ...
      data convec_time,diapfl_total_time /0.0,0.0/
...
c
      if (nstep0.eq.0) then         ! correct initial T/S based on oice
c$OMP PARALLEL DO
        do 17 j=1,jj
        do 17 l=1,isp(j)
        do 17 i=ifp(j,l),ilp(j,l)
         if (oice(i,j).gt. .95 .and. temp(i,j,1).gt. .0)
...
css     if (oice(i,j).gt. .3 .and. i.ge.xpivn) then
        if (oice(i,j).gt. .5) then
          ...
          saln(i,j,k   )=sofsig(th3d(i,j,k)+thbase,temp(i,j,k))
...
c
      write(strg,'(i4.4)') nstep
      call glbint(mm,nn,strg)
c
      sum1=0.
      sum2=0.
      do 501 k=1,kk
      ...
      sum1=sum1+temp(i,j,km)*dp(i,j,km)*scp2(i,j)/onem
      sum2=sum2+temp(i,j,kn)*dp(i,j,kn)*scp2(i,j)/onem
 501  continue
      write(*,'(a,i6,2f12.5)')
...
c
      ...
      do 502 j=1,jj
      do 502 l=1,isp(j)
      do 502 i=ifp(j,l),ilp(j,l)
c     sum1=sum1+temp(i,j,km)*dp(i,j,km)*scp2(i,j)/onem
c     sum2=sum2+temp(i,j,kn)*dp(i,j,kn)*scp2(i,j)/onem
      sum1=sum1+u(i,j,kn)
      sum2=sum2+v(i,j,kn)
      sum3=sum3+dp(i,j,kn)
 502  continue
      write(*,'(a,i6,3f20.16)')
     .   ' htst2=',nstep-nstep0,sum1*1.e-4,sum2*1.e-4,sum3*1.e-12
c
c - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      if (diag_ape)
...
c
      ...
      do 503 j=1,jj
      do 503 l=1,isp(j)
      do 503 i=ifp(j,l),ilp(j,l)
      sum1=sum1+temp(i,j,km)*dp(i,j,km)*scp2(i,j)/onem
      sum2=sum2+temp(i,j,kn)*dp(i,j,kn)*scp2(i,j)/onem
 503  continue
      write(*,'(a,i6,2f12.5)')
     .   ' htst3=',nstep-nstep0,sum1*spcifh*1000/area*1.e-8
     .  ,sum2*spcifh*1000/area*1.e-8
c
      if (diag_ape)
...
ccc      call comparall(m,n,mm,nn,string)
c
      call glbint(mm,nn,'bart')
c
      before = after
      convec_time = 0.0
      call convec(m,n,mm,nn,k1m,k1n)
      call system_clock(after)
      convec_time = real(after-before,4)/real(rate,4)
      convec_total_time = convec_total_time + convec_time
      total_time = total_time + convec_time
...
ccc      call comparall(m,n,mm,nn,string)
c
      call glbint(mm,nn,'conv')
c
      before = after
      diapfl_time = 0.0
      call diapfl(m,n,mm,nn,k1m,k1n)
      call system_clock(after)
      diapfl_time = real(after-before,4)/real(rate,4)
      diapfl_total_time = diapfl_total_time + diapfl_time
      total_time = total_time + diapfl_time
...
c
        ...
        call findmx(maska,sss,iia,iia,jja,'asss')
...
c
      before = after
      call system_clock(after)
      user_time =  cnuity_time+tsadvc_time+convec_time+diapfl_time+
...
c     convec_avg_time=(convec_total_time-convec_time)/(nstep-nstep0-1)
c     diapfl_avg_time=(diapfl_total_time-diapfl_time)/(nstep-nstep0-1)
...
c     write (lp,99009) convec_avg_time,' convec Avg Time at ', nstep-1
c     write (lp,99009) diapfl_avg_time,' diapfl Avg Time at ', nstep-1
...
css   call iceo2a(omlhc,mlhc)
      ...
      call ssto2a(osiav,utilb)                 !kg/m*m per agcm time step
...
c$OMP PARALLEL DO PRIVATE(tf) 
      ...
      if (focean(ia,ja).gt.0.) then
        ...
        dmsi(1,ia,ja)=utilb(ia,ja)                        !kg/m2 per agcm step
        dhsi(1,ia,ja)=utilb(ia,ja)                        !J/m2 per agcm step
...
c$OMP END PARALLEL DO
     ...
     .    ,(itime+1.)/nday,time
      write(800,'(a)') ' end of hycom'
      write(800,'(8f13.8)') ((dp(115,170,k)/onem),k=1,2*kk)
      return
      end
c
      subroutine glbint(mm,nn,text)
c
c --- compute global t/s integrals
c
      implicit none
      include 'dimensions.h'
      include 'dimension2.h'
      include 'common_blocks.h'
c
      real tempmj(jdm),salnmj(jdm),tempnj(jdm),salnnj(jdm),
     .     vlumej(jdm),totemm,tosalm,totemn,tosaln,totvol
     .    ,totvlm,vlummj(jdm)
      character text*(*)
c
c$OMP PARALLEL DO
...
c     tempmj(j)=tempmj(j)+temp(i,j,k+mm)*dp(i,j,k+mm)*scp2(i,j)
c     salnmj(j)=salnmj(j)+saln(i,j,k+mm)*dp(i,j,k+mm)*scp2(i,j)
      tempmj(j)=tempmj(j)+u(i,j,k+nn)*dp(i,j,k+nn)*scp2(i,j)*1.e4
      salnmj(j)=salnmj(j)+v(i,j,k+nn)*dp(i,j,k+nn)*scp2(i,j)*1.e4
      tempnj(j)=tempnj(j)+temp(i,j,k+nn)*dp(i,j,k+nn)*scp2(i,j)
      salnnj(j)=salnnj(j)+saln(i,j,k+nn)*dp(i,j,k+nn)*scp2(i,j)
      vlumej(j)=vlumej(j)               +dp(i,j,k+nn)*scp2(i,j)
      vlummj(j)=vlummj(j)               +dp(i,j,k+mm)*scp2(i,j)
 1    continue
c$OMP END PARALLEL DO
c
      totemm=0.
      ...
      totvlm=0.
      do 2 j=1,jj
      totemm=totemm+tempmj(j)
      tosalm=tosalm+salnmj(j)
      totemn=totemn+tempnj(j)
      tosaln=tosaln+salnnj(j)
      totvol=totvol+vlumej(j)
      totvlm=totvlm+vlummj(j)
 2    continue
c
      write (301,100) text,totemm/totvol,totemn/totvol,
     .      tosalm/totvol,tosaln/totvol,totvol*1.e-22,totvlm*1.e-22
 100  format (a,'  t:',2f15.11,'  s:',4f15.11)
      return
      end
c
c> Revision history:
