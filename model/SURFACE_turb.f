...
c       T(I,J,1)=  T(I,J,1)+DTH1(I,J)
c       Q(I,J,1)=  Q(I,J,1)+DQ1(I,J)
...
c       KMAX=KMAXJ(J)
c       HEMI=1.
c       IF(J.LE.JM/2) HEMI=-1.
c       DO I=1,IMAX
c       DO K=1,KMAX
c         U(IDIJ(K,I,J),IDJJ(K,J),1)=U(IDIJ(K,I,J),IDJJ(K,J),1) -
c    *           RAVJ(K,J)*(DU1(I,J)*COSIV(K)+DV1(I,J)*SINIV(K)*HEMI)
c         V(IDIJ(K,I,J),IDJJ(K,J),1)=V(IDIJ(K,I,J),IDJJ(K,J),1) -
c    *           RAVJ(K,J)*(DV1(I,J)*COSIV(K)-DU1(I,J)*SINIV(K)*HEMI)
c       END DO
c       END DO
c     END DO
C**** non polar boxes
