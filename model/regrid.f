...
c      
c***  loop on points in target (CS) domain
c
      do j=j0_cs,j1_cs
         do i=i0_cs,i1_cs

c     find (ilon,jlat) indices of latlon cell containing the mid-point of the right edge of the (i,j) CS cell
            lon_mid=0.5*(lon2d_corner(i+1,j+1)+lon2d_corner(i+1,j))
     &           /radian
            lat_mid=0.5*(lat2d_corner(i+1,j+1)+lat2d_corner(i+1,j))
     &           /radian

            i_lon = nint(im_ll/2 + 1  + (lon_mid+.01)/dlon_dg )
            j_lat = nint(jm_ll/2 + 1  + (lat_mid+.01)/dlat_dg )
...
c 100        format(A,3(1X,F8.2))
            ...
            if ( I1 .ne. 0 .and. I2 .ne. im_ll+1) then          
               u11 = uin_loc(I1,J1)
               u21 = uin_loc(I2,J1)
               u12 = uin_loc(I1,J2)
               u22 = uin_loc(I2,J2)
c     periodic boundary conditions in x direction
            ...
            if (j_lat .ge. j0_ll .and. j_lat .le. j1_ll) then
...
c     find (ilon,jlat) indices of latlon cell containing the mid-point of the right edge of the (i,j) CS cell
            lon_mid=0.5*(lon2d_corner(i+1,j+1)+lon2d_corner(i,j+1))
     &           /radian
            lat_mid=0.5*(lat2d_corner(i+1,j+1)+lat2d_corner(i,j+1))
     &           /radian
...
c     find (ilon,jlat) indices of latlon cell containing the current CS cell center
            i_lon = nint(im_ll/2 + 1  + (lon2d_dg(i,j)+.01)/dlon_dg )
            j_lat = nint(jm_ll/2 + 1  + (lat2d_dg(i,j)+.01)/dlat_dg )
...
!@auth Denis Gueyffier
     ...
     &     aout_glob(IM_CS,JM_CS)
      ...
      real*8 :: lon1,lon2,lat1,lat2,lon_cs,lat_cs,lat_ll,lon_ll,fjeq
      real*8 :: dnm, pi, dlat_dg, dlon_dg,lat_curr,lon_curr
      real*8 :: ll(2),lon_corner,lat_corner
      integer :: i,j,i_lon,j_lat,im_lon,jm_lat,i1,i2,j1,j2, 
     &     i0_ll,i1_ll,j0_ll,j1_ll,i0_cs,i1_cs,j0_cs,j1_cs
      integer :: ij(2),ij1(2),ij2(2),ij3(2),ij4(2),i_cs,j_cs
      logical :: in1,in2,in3,in4
...
c     find (ilon,jlat) indices of latlon cell containing the upper left corner of current CS cell center
            ...
            i_lon = nint(im_ll/2 + 1  + (lon_corner+.01)/dlon_dg )
            j_lat = nint(jm_ll/2 + 1  + (lat_corner+.01)/dlat_dg )
