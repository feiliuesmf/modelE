...
!@+    RESOLUTION DEPENDENT: This version is for 72x46 - M, 9ky topography
!@+    Fury+Hecla blocked.
...
C**** Nares: (22,43) to (24,44)
      SF(43,L,1) = SF(43,L,1) + OLNST(L+1,1)*FACST
C**** Gibralter: (35,32) to (37,33)
      SF(32,L,1) = SF(32,L,1) + OLNST(L+1,2)*FACST
C**** English: (36,36) to (37,37)
      SF(36,L,1) = SF(36,L,1) + OLNST(L+1,3)*FACST
C**** Bosporous: (42,33) to (43,34)
      SF(33,L,1) = SF(33,L,1) + OLNST(L+1,5)*FACST
C**** Red Sea: (44,29) to (45,28)
      SF(28,L,3) = SF(28,L,3) - OLNST(L+1,6)*FACST
C**** Bab-al-Mandab: (45,28) to (46,27)
      SF(27,L,3) = SF(27,L,3) - OLNST(L+1,7)*FACST
C**** Hormuz: (47,30) to (49,29)
      SF(29,L,3) = SF(29,L,3) - OLNST(L+1,8)*FACST
C**** Korea: (62,32) to (63,33)
      SF(32,L,2) = SF(32,L,2) + OLNST(L+1,10)*FACST
C**** Soya: (64,34) to (65,35)
      SF(34,L,2) = SF(34,L,2) + OLNST(L+1,11)*FACST
C**** Malacca: (56,25) to (58,24), from Indian Ocean to Pacific Ocean
      SF(24,L,2) = SF(24,L,2) - OLNST(L+1, 9)*FACST  !*.5
c      DO 510 J=1,23
c      SF(J,L,2) = SF(J,L,2) - OLNST(L+1, 9)*FACST*.5
c  510 SF(J,L,3) = SF(J,L,3) + OLNST(L+1, 9)*FACST*.5
C****
      RETURN
      END
...
C**** Nares: (22,43) to (24,44)
      IF (J.eq.44) SF(22,44) = SF(22,44) + SUM(OLNST(1:LMST(1),1))*FACST
      IF (J.eq.44) SF(23,44) = SF(23,44) + SUM(OLNST(1:LMST(1),1))*FACST
C**** Gibralter: (35,32) to (37,33)
      IF (J.eq.33) SF(35,33) = SF(35,33) + SUM(OLNST(1:LMST(2),2))*FACST
      IF (J.eq.33) SF(36,33) = SF(36,33) + SUM(OLNST(1:LMST(2),2))*FACST
C**** English: (36,36) to (37,37)
      IF (J.eq.37) SF(36,37) = SF(36,37) + SUM(OLNST(1:LMST(3),3))*FACST
C**** Bosporous: (42,33) to (43,34)
      IF (J.eq.34) SF(42,34) = SF(42,34) + SUM(OLNST(1:LMST(5),5))*FACST
C**** Red Sea: (44,29) to (45,28)
      IF (J.eq.29) SF(44,29) = SF(44,29) + SUM(OLNST(1:LMST(6),6))*FACST
C**** Bab-al-Mandab: (45,28) to (46,27)
      IF (J.eq.28) SF(45,28) = SF(45,28) + SUM(OLNST(1:LMST(7),7))*FACST
C**** Hormuz: (47,30) to (49,29)
      IF (J.eq.30) SF(47,30) = SF(47,30) + SUM(OLNST(1:LMST(8),8))*FACST
      IF (J.eq.30) SF(48,30) = SF(48,30) + SUM(OLNST(1:LMST(8),8))*FACST
C**** Korea: (62,32) to (63,33)
      IF (J.eq.33) SF(62,33) = SF(62,33)+SUM(OLNST(1:LMST(10),10))*FACST
C**** Soya: (64,34) to (65,35)
      IF (J.eq.35) SF(64,35) = SF(64,35)+SUM(OLNST(1:LMST(11),11))*FACST
C**** Malacca: (56,25) to (58,24),
      IF (J.eq.25) SF(56,25) = SF(56,25)+SUM(OLNST(1:LMST( 9), 9))*FACST
      IF (J.eq.25) SF(57,25) = SF(57,25)+SUM(OLNST(1:LMST( 9), 9))*FACST
C****
      RETURN
      END
...
cC**** Fury & Hecla: (19,42) to (20,40)
c      X(40,1,KQ) = X(40,1,KQ) - SOLNST(1)*SCALE
c      X(40,4,KQ) = X(40,4,KQ) - SOLNST(1)*SCALE
c      X(41,1,KQ) = X(41,1,KQ) - SOLNST(1)*SCALE
c      X(41,4,KQ) = X(41,4,KQ) - SOLNST(1)*SCALE
C**** Nares: (22,43) to (24,44)
      X(43,1,KQ) = X(43,1,KQ) + SOLNST(1)*SCALE
      X(43,4,KQ) = X(43,4,KQ) + SOLNST(1)*SCALE
C**** Gibralter: (35,32) to (37,33)
      X(32,1,KQ) = X(32,1,KQ) + SOLNST(2)*SCALE
      X(32,4,KQ) = X(32,4,KQ) + SOLNST(2)*SCALE
C**** English: (36,36) to (37,37)
      X(36,1,KQ) = X(36,1,KQ) + SOLNST(3)*SCALE
      X(36,4,KQ) = X(36,4,KQ) + SOLNST(3)*SCALE
C**** Bosporous: (42,33) to (43,34)
      X(33,1,KQ) = X(33,1,KQ) + SOLNST(5)*SCALE
      X(33,4,KQ) = X(33,4,KQ) + SOLNST(5)*SCALE
C**** Red Sea: (44,29) to (45,28)
      X(28,3,KQ) = X(28,3,KQ) - SOLNST(6)*SCALE
      X(28,4,KQ) = X(28,4,KQ) - SOLNST(6)*SCALE
C**** Bab-al-Mandab: (45,28) to (46,27)
      X(27,3,KQ) = X(27,3,KQ) - SOLNST(7)*SCALE
      X(27,4,KQ) = X(27,4,KQ) - SOLNST(7)*SCALE
C**** Hormuz: (47,30) to (49,29)
      X(29,3,KQ) = X(29,3,KQ) - SOLNST(8)*SCALE
      X(29,4,KQ) = X(29,4,KQ) - SOLNST(8)*SCALE
C**** Korea: (62,32) to (63,33)
      X(32,2,KQ) = X(32,2,KQ) + SOLNST(10)*SCALE
      X(32,4,KQ) = X(32,4,KQ) + SOLNST(10)*SCALE
C**** Soya: (64,34) to (65,35)
      X(34,2,KQ) = X(34,2,KQ) + SOLNST(11)*SCALE
      X(34,4,KQ) = X(34,4,KQ) + SOLNST(11)*SCALE
C****
C**** Calculate transport through straits from one basin to another
C****
C**** Malacca: (56,25) to (58,24)
c      DO 510 J=1,23
c      X( J,2,KQ) = X( J,2,KQ) + SOLNST( 9)*SCALE
c  510 X( J,3,KQ) = X( J,3,KQ) - SOLNST( 9)*SCALE
      X(24,2,KQ) = X(24,2,KQ) - SOLNST( 9)*SCALE
      X(24,4,KQ) = X(24,4,KQ) - SOLNST( 9)*SCALE
