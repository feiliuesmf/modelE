...
c
      if (nstep0.eq.0) then         
c$OMP PARALLEL DO
        do 17 j=1,jj             ! correct initial T/S based on oice
        do 17 l=1,isp(j)
...
cd    nsaveo=nsaveo+1
 15   continue
cd    print *,' chk ogcm saved over hr=',nsaveo*baclin/3600
cd    nsaveo=0
c
c       before = after
...
cd    write(*,'(a,2f10.1)') 'chk agcm/ogcm date at end of ogcm'
cd   .    ,(itime+1.)/nday,time
      return
      end
c
c> Revision history:
