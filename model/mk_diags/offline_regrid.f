      subroutine offline_regrid(x2grids,tsource,ttarget)

!@sum  regrids data from source grid to target grid
!@auth Denis Gueyffier
      use offregrid_com
      implicit none
      type (x_2gridsoff), intent(in) :: x2grids
...
c     ll2cs
     ...
     &                    /atarget(i,j,itile)
               ...
               enddo
            enddo
      elseif ((x2grids%ntilessource .eq. 6) .and.
...
c     cs2ll
            ...
            do n=1,x2grids%xgrid%ncells
               ...
               atarget(il,jl,1) = atarget(il,jl,1)
     &              + x2grids%xgrid%xgrid_area(n)
               ttarget(il,jl,1) = ttarget(il,jl,1)
     &              + x2grids%xgrid%xgrid_area(n)
     &              *tsource(icc,jcc,itile)
            enddo

               do j=1,x2grids%jmtarget
                  do i=1,x2grids%imtarget
                     ttarget(i,j,1) = ttarget(i,j,1)
     &                    /atarget(i,j,1)
                  enddo
               enddo
      ...
      end subroutine offline_regrid
c*

      subroutine init_offregrid(x2grids,imsource,jmsource,
     &     ntilessource,imtarget,jmtarget,ntilestarget)

!@sum  Reads regriding file and create 
...
!@auth Denis Gueyffier
      ...
      include 'netcdf.inc'
      include 'mpif.h'
      type (x_2gridsoff), intent(inout) :: x2grids
      type (x_grid) :: xgrid
