...
C**** Update halos of U and V
      ...
      if(have_south_pole) then
        ...
        do l=1,lm
          ua(l,1,1) =
     &         (sum(u(:,jv,l)*cosiv(:))-hemi*sum(v(:,jv,l)*siniv(:)))/im
          va(l,1,1) =
     &         (sum(v(:,jv,l)*cosiv(:))+hemi*sum(u(:,jv,l)*siniv(:)))/im
        enddo
      endif
     ...
     &         (sum(v(:,jv,l)*cosiv(:))+hemi*sum(u(:,jv,l)*siniv(:)))/im
      ...
      do j=j_0s,j_1s
        ...
        do l=1,lm
          ua(l,i,j) =
     &         .25*(u(im,j,l)+u(i,j,l)+u(im,j+1,l)+u(i,j+1,l))
          va(l,i,j) =
     &         .25*(v(im,j,l)+v(i,j,l)+v(im,j+1,l)+v(i,j+1,l))
        enddo
        do i=2,im
          do l=1,lm
            ua(l,i,j) =
     &           .25*(u(i-1,j,l)+u(i,j,l)+u(i-1,j+1,l)+u(i,j+1,l))
            va(l,i,j) =
     &           .25*(v(i-1,j,l)+v(i,j,l)+v(i-1,j+1,l)+v(i,j+1,l))
          enddo
        enddo
      enddo
      return
