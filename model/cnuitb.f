...
c
      ...
      call cpy_p(pold)
c
c$OMP PARALLEL DO PRIVATE(ja,q) SHARED(k)
      do 12 j=1,jj
      ja=mod(j-2+jj,jj)+1
c
      do 11 l=1,isu(j)
...
c
      ...
      call cpy_p(util2)
c
c$OMP PARALLEL DO PRIVATE(ja,clip) SHARED(k)
      do 29 j=1,jj
      ja=mod(j-2+jj,jj)+1
c
      do 28 l=1,isu(j)
...
c
      ...
      call cpy_p(util2)
c
c$OMP PARALLEL DO PRIVATE(flxhi,flxlo,ja,jb)
c$OMP. SHARED(k)
      do 16 j=1,jj
      ja=mod(j-2+jj,jj)+1
      jb=mod(j     ,jj)+1
...
c
c$OMP PARALLEL DO PRIVATE(jb) SHARED(k,kn)
      do 261 j=1,jj
      jb=mod(j     ,jj)+1
      do 261 l=1,isp(j)
...
c
c$OMP PARALLEL DO PRIVATE(ja,q) SHARED(k,kn)
      do 145 j=1,jj
      ja=mod(j-2+jj,jj)+1
c
      do 144 l=1,isu(j)
