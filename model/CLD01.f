...
C**** ACCUMULATE SOME DIAGNOSTICS
  ...
  304    AJ11(L)=AJ11(L)+(TNEW-TOLD)*AIRM(L)*BYDSIG(L)
C****
...
C**** RE-EVAPORATION OF LWC IN THE UPPER LAYER
      ...
      TNEWU=TL(L+1)
         ...
         AJ11(L)=AJ11(L)+(TNEW-TOLD)*AIRM(L)*BYDSIG(L)
         AJ11(L+1)=AJ11(L+1)+(TNEWU-TOLDU)*AIRM(L)*BYDSIG(L)
         AJ55(L)=AJ55(L)+(QNEW-QOLD)*AIRM(L)*BYDSIG(L)*LHX*BYSHA
         AJ55(L+1)=AJ55(L+1)+(QNEWU-QOLDU)*AIRM(L)*BYDSIG(L)*LHX*BYSHA
  382 CONTINUE
         WMSUM=0.
