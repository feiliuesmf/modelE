...
C****
     ...
     &     , only : im,jm
      ...
      REAL*8 STBO,C
      INTEGER NEXP
      PARAMETER (STBO=5.67032E-8)
      PARAMETER (NEXP=6)
      PARAMETER (C=2.3025851)
C**** INCLUDE FILE FOR SOILS45
      REAL*8 A1,A2,A3,AA,ABETA,ABETAB,ABETAD,ABETAP,ABETAT,ABETAV,ACE2AV
     *     ,ACNA,ACNC,ACS,ADIFS,AEDIFS,AEPB,AEPC,AEPP,AERUNS,AERUNU
     *     ,AEVAPB,AEVAPD,AEVAPW,AF0DT,AF1DT,AFB,AFHG,AFR,AK1,AK2,ALA
     *     ,ALAI,ALAIC,ALAIE,ALAMA,ALAMBR,ALAMI,ALAMS,ALAMSN,ALAMW
     *     ,ALGDEL,ALHG,ALPH0,ALPH0O,ALPLS1,ARG,ARUNS,ARUNU,ASHG,ATRG
     *     ,AVH,BA,BETA,BETAB,BETAD,BETADL,BETAS,BETAT,BETAV,C1,CC,CH
     ...
     *     ,FJEQ,FLMT,FM,FR,FRDN,FRUP,FSN,FTP,FUNC,FV,FW,GAA,GABC,GAMMA
     ...
     *     ,TESTH,TFRZ,TG2AV,THETA,THETM,THETS,THM,THR,THR0,THR1,THR2
     *     ,THRM,TOL,TP,TPC,TRUNC,TRHT,TS,TWOPI,U1,V1,VH,VSM,W,WC,WET1
     *     ,WET2,WFC1,WFC2,WMIN,WS,WTR2AV,XA,XB,XDEN,XI,XINFC,XK,XK1,XK2
     ...
     *     ,IZ,IZX
     ...
     & ,DTS,SHTPR,DTOUT,TFRZ,BETAD,HW,DR,DRS,RS,PRFR,C1,AL(2),XINC
...
C     COMMON/WEIGHT/A(4,IMT-1),B(4,IMT-1),P(4,IMT-1)
     ...
     &     (/4,imt-1/))
      COMMON/SOILS2/SDATA(IM,JM,11*NGM+1),ALA(3,IM,JM),ACS(3,IM,JM)
     &   ,AFB(IM,JM),AFR(IM,JM,NGM),AVH(IM,JM)
...
C****
      RETURN
      END SUBROUTINE GDTM
      SUBROUTINE GHINIT (FDATA,VDATA,DTSURF,SHCLC0)
C**** Modifications needed for split of bare soils into 2 types
      !INCLUDE 'soils45.COM'
C**** SOILS28   Common block     9/25/90
      REAL*8 FDATA
      REAL*8 VDATA,DTSURF,SHCLC0
      DIMENSION FDATA(IM,JM,3),VDATA(IM,JM,10)
...
C SPGSN IS THE SPECIFIG GRAVITY OF SNOW
      SPGSN=.1
C
CR    CALL SOLDAT (FDATA,VDATA)
C****
C**** Initialize global arrays  ALA, ACS, AFB, AFR
C****
      TWOPI=6.283185
      FJEQ=(JM+1)/2.
      DO 210 I=1,IM*JM*(8+NGM)
  210 ALA(I,1,1)=0.
      DO 220 J=1,JM
      DO 220 I=1,IM
  220 ACS(1,I,J)=.01
...
C**** WFC1 - WATER FIELD CAPACITY OF TOP SOIL LAYER, M
C****
      !INCLUDE 'soils45.COM'
C**** SOILS28   Common block     9/25/90
      INTEGER I0,J0
...
C**** BASED ON COMBINATION OF LAYERS 2-N, AS IN RETP2
   ...
   30 CONTINUE
      ...
      WFC2=1000.*WFC2
      FICE(0,2)=1.
      FICE(1,1)=(ACE1+SNOWDP*1000.)/(WTR1+ACE1+SNOWDP*1000.+1.E-20)
      FICE(1,2)=FICE(1,1)
      TP(0,2)=TG1
C**** W = SNOW(IF TOP LAYER) + WMIN + (WMAX-WMIN)*(WTR+ICE)/WFC
      ...
      TP(1,IBV)=TG1
      DO 1 L=2,N
      FICE(L,IBV)=ACE2/(WTR2+ACE2+1.E-20)
