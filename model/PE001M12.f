...
C****
     ...
     *  COSZ1(IM,JM),COSZ2(IM,JM),COSZA(IM,JM),
     ...
     *   TOTCLD(LM)
      PARAMETER (LX=40) ! check with radiation common block
      COMMON/RADCOM/V6ALB(11,4,7),TAUWC0,TAUIC0,EPSCON,RO3COL,FULGAS(13)
     ...
     E             ,KTREND,NTRACE,ITR(8),LMR,LMRP,MLAT46,MLON72,LASTVC
C     INPUT DATA
     F             ,PLE(LX),HLB(LX),TLB(LX),TLT(LX),TL(LX),U0GAS(LX,12)
     G             ,ULGAS(LX,12),QL(LX),RHL(LX),TRACER(LX,8),AERTAU(LX)
     H             ,TAUWC(LX),TAUIC(LX),SIZEWC(LX),SIZEIC(LX),CLDEPS(LX)
     I          ,POCEAN,PEARTH,POICE,PLICE,AGESN(3),SNOWE,SNOWOI,SNOWLI
     J             ,TGO,TGE,TGOI,TGLI,TS ,WS  ,WEARTH,ZOICE,FSPARE(998)
     K             ,S00WM2,RATLS0,S0,COSZ,PVT(11),BXA(153),SRBXAL(15,2)
     L           ,XXJDAY,JYEARR,JDAYR,JLAT,ILON,ISPARE(98),DMOICE,DMLICE
C     OUTPUT DATA
...
C     Some Aerosol Control Parameters
      COMMON/AERCOM/QXAERO(6,9),QSAERO(6,9),QCAERO(6,9),ATAERO(33,9)
     A             ,QAER55(9),REAERO(9),VEAERO(9),ROAERO(9),PI0MAX(9)
     B             ,FSAERO(9),FTAERO(9),VDGAER(12,9),SSBTAU,PIAERO(9)
...
C**** SET THE CONTROL PARAMETERS FOR THE RADIATION (need mean pressures)
   ...
   30 PLE(L)=SIGE(L)*(PSF-PTOP)+PTOP
      ...
      PLE(LMR)=.2*PLE(LM+1)
      PLE(LMR+1)=1.D-5
      FSPARE(31)=PTOP ! top of sigma-coord.system
      DO 40 LR=LM+1,LMR
...
C**** New options (currently not used)
      ISPARE(12)=0  ! 0:old 1:new LW cloud scattering scheme  -  KCLDEM
      ISPARE(16)=0  ! 0:2-band 1:6-band veg.albedo            -  KVEGA6
...
C****
      ...
      INCLUDE 'SOMTQ.COM'
      COMMON U,V,T,P,Q
      COMMON/WORK1/CONV(IM,JM,LM),PK(IM,JM,LM)
