...
c
C**** Local parameters and variables and arguments:
!@var Itime,time model Itime, passed from masterchem
!@var theta solar zenith angle
!@var rlat latitude (FASTJLAT) passed frommasterchem
!@var rlong longitude (FASTJLON) passed from masterchem
!@var long local copy of rlong, so it is not passed back
!@param zone the difference between the observer's clock and
!@+   a clock reading GMT, in decimal hours. For example, if
!@+   the observer is using Eastern Standard Time, zone = -5.
!@param zobs
!@param tem surface temp (K), should really be sent with call
!@param rearth radius of the earth in meters
!@var rtime time/24
!@var LST,LMST,UT,n,L,g,E,LAST,HA,alt,alt2 dummies?
!@var k dummy loop variable
!@var JD related to Julian date
!@var lamdba ECLIPTIC LONGITUDE
!@var alpha RIGHT ASCENSION
!@var delta DECLINATION
!@var epsilon ?
!@var closest distance of closest approach
!@var neq logical: is closest negative?
!@var P,Press reference pressures mb (21)
!@var T,Temp reference temperatures K (11)
...
C
      long = rlong
      time=float(Itime)
      if(abs(rlat).gt.88.)long=0.
c
c Compute altitude (angle from horizontal) of sun, using "Low-precision
c formulae for the Sun's coordinates and the equation of time", The
c Astronomical Almanac, 1986, p C24.
c
      rtime=time/24.
      LST=NINT((rtime-int(rtime))*24)
      UT=LST-zone
      LMST=UT+long/15
c
c     CONVERT DATE TO JULIAN DAY
      JD=2442412.5  !1976 date
      JD=JD+rtime-zone
c
c     CALCULATE ECLIPTIC LONGITUDE (lambda)
      n=JD-2451545.0
      ...
      g=mod(g,360.D0)
      if (g.lt.0.) g=g+360.
      lambda=L+1.915*sin(radian*g)+0.020*sin(radian*2.*g)
c
c     CALCULATE RIGHT ASCENSION (alpha) AND DECLINATION (delta)
      epsilon=23.439-0.0000004*n
      if(mod(lambda,90.D0).eq.0.) then
       alpha=lambda
      else
       alpha=atan(cos(radian*epsilon)*tan(radian*lambda))/radian
       if(lambda.gt.270.) then
        alpha=alpha+360.
       else if (lambda.gt.90.)  then
        alpha=alpha+180.
       endif
      endif
      delta=asin(sin(radian*epsilon)*sin(radian*lambda))/radian
      E=L-alpha
c
c     CONVERT FROM EQUATORIAL TO HORIZON COORDINATES
      LAST=LMST+E/15.
      HA=15.*(LAST-12.)
      alt=asin(sin(radian*rlat)*sin(radian*delta)
     &   +cos(radian*rlat)*cos(radian*delta)*cos(radian*HA))/radian
c
c     CALCULATE DISTANCE OF CLOSEST APPROACH TO EARTH
      neg=.false.
      if (alt.gt.0.) then
        closest=zobs
        goto 19
      endif
      closest=rearth*sin(radian*(90.+alt))-rearth
      if (closest.lt.0.) then
        closest=0.
        neg=.true.
        alt2=alt
      endif
c
c     LOOK UP P(mbar) AND T(K) AT CLOSEST APPROACH
   19 if (closest.gt.20.) then
        theta=90.-alt
        goto 10
      endif
      do 8 k=1,21
        l=k-.5
        if (closest.lt.l) then
          Press=P(k)
          goto 9
        endif
    8 continue
...
c
c     CORRECT FOR ATMOSPHERIC REFRACTION
      if (alt.gt.0.) then
        R=Press*(0.1595+0.0196*alt+0.00002*alt**2.)
        R=R/(Temp*(1.+0.505*alt+0.0845*alt**2.))
      else
        R=(Press*0.1595)/Temp
      endif
      theta=90.-alt
c      if (zobs.gt.40.) then
c        theta=theta-2.*R
c      else
        theta=theta-log(0.125*zobs+2.5)*R
c      endif
      if (neg) then
        if (theta.lt.90.) goto 10
        closest=rearth*sin(radian*(180.-theta))-rearth
        if (closest.lt.0.) then
          theta=90.-alt2
        endif
      endif
c
c     empirical correction for mathematical error near 90 degrees
   10 if(theta.gt.90.and.theta.lt.92.)then
        corr=92.-theta
        corr=corr*2./1.4
        theta=92.-corr
      endif
