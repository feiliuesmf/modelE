...
#undef TRACERS_WATER
#endif
!@sum  GLIMMERICE_DRV contains drivers for GLIMMERICE related routines
!@auth Gavin Schmidt
!@ver  2010/10/13
...
#endif
#endif               /* TNL: inserted */
      Use GLIMMERICE_COM, Only: TLANDI,SNOWLI, MDWNIMP,EDWNIMP,
     *                       FSHGLM,FNHGLM
#ifdef TRACERS_WATER
     *     ,trsnowli,trlndi,trdwnimp
#endif
      USE FLUXES, only : atmgla,gmelt,egmelt
#ifdef TRACERS_WATER
#ifdef TRACERS_OCEAN
     *     ,trgmelt
#endif
      ...
      DO J=J_0,J_1
        DO I=I_0,I_1
          IF (FLICE(I,J).gt.0) THEN
            atmgla%GTEMP(I,J)=TLANDI(1,I,J)
            atmgla%GTEMP2(I,J)=TLANDI(2,I,J)
            atmgla%GTEMPR(I,J)   =TLANDI(1,I,J)+TF
#ifdef SCM
...
!TODO and may be different for each individual tracer (reto) 
            ...
            IF (SNOWLI(I,J).gt.SNMIN) THEN
              atmgla%GTRACER(:,I,J)=TRSNOWLI(:,I,J)/SNOWLI(I,J)
            ELSE
              atmgla%GTRACER(:,I,J)=TRLNDI(:,I,J)/(ACE1LI+ACE2LI)
            END IF
            end if
#endif
          END IF
        ...
        DO I=I_0,IMAXJ(J)
          ...
          IF (LOC_GLM(I,J) .and. FOCEAN(I,J).GT.0.) THEN
            byoarea = 1.d0/(AXYP(I,J)*FOCEAN(I,J))
             GMELT(I,J) =  GMELT(I,J)*byoarea
            EGMELT(I,J) = EGMELT(I,J)*byoarea
#ifdef TRACERS_WATER  /* TNL: inserted */
#ifdef TRACERS_OCEAN
            TRGMELT(:,I,J) = TRGMELT(:,I,J)*byoarea
#endif
#endif /* TNL: inserted */
          ENDIF
        END DO
...
!@calls GLIMMERICE:PRECLI
     ...
     *     ,trunoli,trprec
#endif
      USE GLIMMERICE, only: ace1li,ace2li,precli,snmin
      USE GLIMMERICE_COM, only : snowli,tlandi,mdwnimp,edwnimp
#ifdef TRACERS_WATER
...
#endif
      USE GEOM, only : imaxj,axyp,byaxyp
      USE GLIMMERICE, only : lndice,ace1li,ace2li,snmin,micbimp
...
!@ver  2010/10/13
     ...
     &                             INT_DAYS_PER_YEAR
      ...
      USE MODEL_COM, only : dtsrc,itime,itimei,nday,JDperY
      USE GEOM, only : axyp,imaxj,lat2d
      USE GLIMMERICE, only: ace1li,ace2li,glmelt_on,glmelt_fac_nh
     *     ,glmelt_fac_sh,fwarea_sh,fwarea_nh,accpda,accpdg,eaccpda
