...
!@+    PRECIP_OC,OCEANS
      ...
      USE MODEL_COM, only : Iyear1,Itime,JDendOfM,JDmidOfM,ItimeI,kocean
...
C****   READ IN LAST MONTH'S END-OF-MONTH DATA
        if (ocn_cycl.ge.1 .and. ocn_cycl.le.2) then
          LSTMON=month-1
          if (lstmon.eq.0) lstmon = INT_MONTHS_PER_YEAR
          CALL READT_PARALLEL
     ...
     *           (grid,iu_OSST,NAMEUNIT(iu_OSST),TEMP_LOCAL,LSTMON)
            ...
            LSTMON=month-1+(year-IYEAR1)*INT_MONTHS_PER_YEAR
...
C**** READ IN CURRENT MONTHS DATA: MEAN AND END-OF-MONTH
     ...
     *    WRITE(6,*) 'Read in OSST ocean data for month',month,M
          IF(month.NE.MOD(M-1,INT_MONTHS_PER_YEAR)+1)
     ...
     *  WRITE(6,*) 'Read in SICE ocean data for month',month,M,M1
        IF(M.NE.M1.OR.month.NE.MOD(M-1,INT_MONTHS_PER_YEAR)+1)
     &       call stop_model('Error: SICE ocean data',255)
        if(ocn_cycl>2 .and. month==INT_MONTHS_PER_YEAR) then
          do m=INT_MONTHS_PER_YEAR,0,-1
            CALL BACKSPACE_PARALLEL( iu_OSST )
            CALL BACKSPACE_PARALLEL( iu_SICE )
          end do
        end if
...
c            if (rsinew.gt.rsi(i,j)) ! ice from ocean
c     *           call RESET_SURF_FLUXES(I,J,1,2,RSI(I,J),RSINEW)
c            if (rsinew.lt.rsi(i,j)) ! ocean from ice
c     *           call RESET_SURF_FLUXES(I,J,2,1,1.-RSI(I,J),1.-RSINEW)
C****
            RSI(I,J)=RSINEW
            MSI(I,J)=MSINEW
      ...
      IF(HAVE_NORTH_POLE) THEN
        IF (FOCEAN(1,JM).gt.0) THEN
          DO I=2,IM
            ...
            TRSI(:,:,I,JM)=TRSI(:,:,1,JM)
#endif
            atmice%GTEMP(I,JM)=atmice%GTEMP(1,JM)
            atmice%GTEMP2(I,JM)=atmice%GTEMP2(1,JM)
            atmice%GTEMPR(I,JM)=atmice%GTEMPR(1,JM)
      ...
      IF(HAVE_SOUTH_POLE) THEN
        IF (FOCEAN(1,1).gt.0) THEN
          DO I=2,IM
            ...
            TRSI(:,:,I,1)=TRSI(:,:,1,1)
#endif
            atmice%GTEMP(I,1)=atmice%GTEMP(1,1)
            atmice%GTEMP2(I,1)=atmice%GTEMP2(1,1)
            atmice%GTEMPR(I,1)=atmice%GTEMPR(1,1)
            FWSIM(I,1)=FWSIM(1,1)
          END DO
        END IF
      END IF
     ...
     *           (grid,iu_OCNML,NAMEUNIT(iu_OCNML),XZO,IMON-1)
      ...
      ELSE                      ! Do we need to read in second month?
        IF (DAYOFYEAR.NE.JDLAST+1) THEN ! Check that data is read in daily
          IF (DAYOFYEAR.NE.1 .or. JDLAST.NE.INT_DAYS_PER_YEAR) THEN
...
C**** For the time being assume zero surface velocities for drag calc
        ...
        atmice%uisurf(i,j)=0. ; atmice%visurf(i,j)=0. ! move elsewhere
...
!@auth Original Development Team
      USE CONSTANT, only : twopi,shw,rhows,tf
...
C**** Set fourier coefficients for heat transport calculations
      IF (KOCEAN.ge.1) THEN
        ANGLE=TWOPI*modelEclock%dayOfYear()/EARTH_DAYS_PER_YEAR
        SINANG=SIN(ANGLE)
        SN2ANG=SIN(2*ANGLE)
        SN3ANG=SIN(3*ANGLE)
        SN4ANG=SIN(4*ANGLE)
...
c
     ...
     *     ,SMSI0,ENRGW,WTRW0,WTRW,RUN0,RUN4,ROICE,SIMELT,ESIMELT
      ...
      REAL*8, DIMENSION(:,:), POINTER :: RSI,MSI,SNOWI,PREC,EPREC,
     ...
     &     FWSIM
      ...
      DO I=I_0,IMAXJ(J)
        ...
        IF (FOCEAN(I,J).gt.0) THEN
          ...
          SIMELT =(MELTI(I,J)-SMELTI(I,J))/(FOCEAN(I,J))
          ESIMELT=EMELTI(I,J)/(FOCEAN(I,J))
...
C**** Additional mass (precip) is balanced by deep removal
          ...
          END IF
          GTEMP(I,J)=TOCEAN(1,I,J)
          GTEMPR(I,J) =TOCEAN(1,I,J)+TF
#ifdef SCM
...
!@auth Gavin Schmidt
     ...
     *     ,itocean,itoice
      ...
      REAL*8, DIMENSION(:,:), POINTER :: RSI,MSI,SNOWI,FWSIM
      REAL*8, DIMENSION(:,:,:), POINTER :: HSI,SSI
#ifdef TRACERS_WATER
      REAL*8, DIMENSION(:,:,:,:), POINTER :: TRSI
      INTEGER :: TIJ_ICOCFLX
      ...
      IF (KOCEAN.lt.1) THEN ! prescribed ice: accumulate implied heat conv.
        DO J=J_0,J_1
        DO I=I_0,IMAXJ(J)
          IF (FOCEAN(I,J).gt.0) THEN
            OA(I,J,13)=OA(I,J,13)+atmice%HSICNV(I,J)
          END IF
        END DO
        END DO
      ELSE     ! qflux model
