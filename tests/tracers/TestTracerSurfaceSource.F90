module TestTracerSurfaceSource_mod
  use pfunit
...
contains
  ...
  contains
    ...
    subroutine check(day, monthA, monthB, frac)
      ...
      call assertEqual(monthA, monthA_found, 'monthA')
      call assertEqual(monthB, monthB_found, 'monthB')
      call assertEqual(frac, frac_found, 'frac')
  ...
  contains
    ...
    subroutine check(dayOfYear, year, ty_start, ty_end, delTyr, msg)
      ...
      call assertEqual(alpha, alpha_found,'alpha' // msg)
      call assertEqual(frac, frac_found,'frac' //  msg)

      if (alpha /= 1.) then
        call assertEqual(record(1,1), 1+monthA+offset1, 'record(1,1) monthA:yearA' // msg)
        call assertEqual(record(2,1), 1+monthB+offset1, 'record(2,1) monthB:yearA' // msg)
      end if
      if (alpha /= 0.) then
        call assertEqual(record(1,2), 1+monthA+offset2, 'record(1,2) monthA:yearB' // msg)
        call assertEqual(record(2,2), 1+monthB+offset2, 'record(2,2) monthB:yearB' // msg)
      end if
...
end module TestTracerSurfaceSource_mod
