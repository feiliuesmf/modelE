...
C**** NEWER VERSION THAT EXPECTS ARRAYS ON INDIVIDUAL RECORDS
      ...
      PARAMETER (IM=72,JM=46,LM=12,NGM=6,KTD=8,KAIJ=100,KAJK=51,KCON=54)
      parameter (npbl=8)
      PARAMETER (IMH=IM/2,
     *     KACC=JM*94*6 + 24*94 +
     *     JM*3 +JM*LM*57 + JM*3*4 + IM*JM*KAIJ + IM*LM*16 +IM*JM*29 +
     *     20*100 + JM*KCON + (IMH+1)*20*8 +8*2 + 24*63*4 + 2*62*10*12 +
     *     JM*LM*KAJK +IM*JM*LM*6 + IM*JM*LM*5 + JM*LM*10*3)
      ...
      COMMON/BNDYCB1/TOCEAN1(3,IM,JM),GDATA1(IM,JM,7),BLDATA1(IM,JM,11)
     *     ,Z1(IM,JM)
      COMMON/BNDYCB2/TOCEAN2(3,IM,JM),GDATA2(IM,JM,7),BLDATA2(IM,JM,11)
     *     ,Z2(IM,JM)
ccc   ice data:
...
C****
      ...
      OPEN (1,FILE=FILEIN,FORM='UNFORMATTED',STATUS='OLD',ERR=810)
         ...
         READ (1) HEADER,KEYNR,TSFREZ1,DIAG1,TDIURN1,ITAU2
...
C****
      ...
      OPEN (2,FILE=FILEIN,FORM='UNFORMATTED',STATUS='OLD',ERR=810)
         ...
         READ (2) HEADER,KEYNR,TSFREZ2,DIAG2,TDIURN2,ITAU2
